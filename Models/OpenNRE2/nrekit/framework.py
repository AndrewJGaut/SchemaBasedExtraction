...
saver.restore(self.sess, ckpt)
	...
	'''per relation stats'''
	tot_spouse = 0
	tot_spouse_correct = 0
	tot_birthdate = 0
	tot_birthdate_correct = 0
	tot_birthplace = 0
	tot_birthplace_correct = 0
	tot_hypernym = 0
	tot_hypernym_correct = 0
	...
	'''per relation stats!'''
	iter_spouse_correct = np.logical_and(iter_output == batch_data['rel'], batch_data['rel'] == 5).sum()
	tot_spouse_correct += iter_spouse_correct
	tot_spouse += np.logical_and(batch_data['rel'], batch_data['rel'] == 5).sum()

	iter_birthdate_correct = np.logical_and(iter_output == batch_data['rel'], batch_data['rel'] == 3).sum()
	tot_birthdate_correct += iter_birthdate_correct
	tot_birthdate += np.logical_and(batch_data['rel'], batch_data['rel'] == 3).sum()

	iter_birthplace_correct = np.logical_and(iter_output == batch_data['rel'], batch_data['rel'] == 2).sum()
	tot_birthplace_correct += iter_birthplace_correct
	tot_birthplace += np.logical_and(batch_data['rel'], batch_data['rel'] == 2).sum()
		...
		if(tot_birthdate > 0):
			...
			if(tot_birthplace > 0):
				...
				if(tot_hypernym > 0):
					...
					print('SPOUSE: numcorrect={}, numtot={}, percentage={}'.format(tot_spouse_correct, tot_spouse, spouse_percentage))
					print('BIRTHDATE: numcorrect={}, numtot={}, percentage={}'.format(tot_birthdate_correct, tot_birthdate, birthdate_percentage))
					print('BIRTHPLACE: numcorrect={}, numtot={}, percentage={}'.format(tot_birthplace_correct, tot_birthplace, birthplace_percentage))
					print('HYPERNYM: numcorrect={}, numtot={}, percentage={}'.format(tot_hypernym_correct, tot_hypernym, hypernym_percentage))
