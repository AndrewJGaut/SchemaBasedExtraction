...
def onlyKeepDoubleEntitySentences(dataset_path):
    ...
    output_write_row = 1  # skip first row with column headers
    # num_eliminated = [0] * sheet.ncols
        ...
        if write:
            for k in range(1, len(matrix_to_write)):
                for j in range(len(matrix_to_write[k])):
                    if matrix_to_write[k][j] != "":
                        curr_row = k
                        while (curr_row > 0 and matrix_to_write[curr_row - 1][j] == ""):
                            matrix_to_write[curr_row - 1][j] = matrix_to_write[curr_row][j]
            ...
            for k in range(0, len(matrix_to_write) - num_empty_rows):
                for j in range(len(matrix_to_write[k])):
                    new_sheet.write(output_write_row, j, matrix_to_write[k][j])
                output_write_row += 1
        already_wrote = True
...
def cleanDataset(dataset_path):
    eliminateRepeatedSentences(dataset_path)
    onlyKeepDoubleEntitySentences(dataset_path[0:-5] + "_fixed.xls")


'''
def takeOutSpaces(dataset_path):
    dataset = xlrd.open_workbook(dataset_path)
    sheet = dataset.sheet_by_index(0)

    new_dataset = xlwt.Workbook()
    new_sheet = new_dataset.add_sheet('No Spaces')

    # copy over header stuff
    for j in range(sheet.ncols):
        new_sheet.write(0, j, sheet.cell_value(0, j))
...
if __name__ == '__main__':
    ...
    cleanDataset('female_negative_examples.xls')
